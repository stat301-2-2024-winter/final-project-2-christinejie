...
#| echo: false
#| label: tbl-summary
#| tbl-cap: Summary of All Variables Using Skim 
skimr::skim_without_charts(house)
```
From @tbl-summary, there are no variables with any missingness issues. The variables `city`, `has_cooling` (has cooling), `has_heating` (has heating), `has_spa` (has spa) have class imbalance issues, which I addressed by removing them from my recipe. 
...
**Model Types**: I used the 6 following models: 

1) Baseline: This is my most basic model. 

2) Boosted Tree (bt): Boosted Trees work by putting together a bunch of decision trees, where each new tree is trained to correct the mistakes of the previous ones. Then, all these trees are added together to make the final prediction, with each tree's contribution adjusted based on how much it helps improve the overall prediction.

3) Elastic Net (en): The Elastic Net model combines two types of penalties (Lasso and Ridge) to perform linear regression, using both feature selection (picking out the most important information from a bunch of data to make predictions) and coefficient shrinkage (preventing the model from overreacting to noisy data) for better prediction accuracy. 

4) K - Nearest Neighbors (knn): The knn model selects a number of features for prediction, calculates the Euclidean distance between the new data point and each training point, and then finds the average value of the "K" closest data points for prediction.

5) Simple Linear (lm): the linear model describes the relationship between a dependent variable and independent variables with a straight line, determined by a slope and y-intercept, often estimated using the method of least squares.

6) Random Forest (rf): the random forest model creates many  decision trees which are built on a random sample of the data, and features that are randomly chosen.Then, for predictions, it combines the results of all the trees, either by averaging them for numerical data or by picking the most popular choice for categorical data 

**Tuning Parameters**
Tuning a model adjusts its parameters to optimize its performance on a given dataset. This involves experimenting with different parameter values to find the combination that results in the lowest RMSE.  
...
For the Elastic Net model, I tuned the penalty and mixture parameters. Penalty is a term added during training to discourage overfitting. Mixture controls whether the elastic model is more like a ridge model (mixture = 0) or more like a lasso model (mixture = 1). 
...
1) Standard kitchen sink recipe where I used all of the predictor variables. 

2) Standard Feature Engineered recipe where I removed variables that had only a few levels which had little predictive power or severe class imbalance issues. I also interacted the variables for number of bathrooms and number of bedrooms to account for dependencies between these variables. (See appendix for EDA)

3) Tree based kitchen sink recipe where I used all of the predictor variables. 

4) Tree based Feature Engineered recipe where I removed variables that had only a few levels which had little predictive power or severe class imbalance issues. 
...
```{r}
#| echo: false
#| label: tbl-best-param
#| tbl-cap: Best Performing Parameters for Each Model
best_parameters
```
From @tbl-best-param, we can see the best performing parameters for each model.  
...
#| echo: false
#| label: tbl-not-log
#| tbl-cap: RMSE, RSQ, MAE of Final Model in Log Base 10 
house_final_metrics_log
```
From @tbl-not-log, we can see the RMSE, RSQ, MAE of the final winning model in Log Base 10. 

The RMSE measures the average magnitude of difference between values predicted by a model and the actual values. Here, the RMSE is 0.114 which means the average magnitude of difference between predicted and actual log10(price) is 0.114. This is much lower and better than the baseline model, which had a RMSE of 0.240. This means that building a predictive model did pay off. 
...
Pierce, E. (2021). Austin, TX House Listings. Kaggle.com. [https://www.kaggle.com/datasets/ericpierce/austinhousingprices](https://www.kaggle.com/datasets/ericpierce/austinhousingprices)
